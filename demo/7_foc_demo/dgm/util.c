...
static const unsigned int crc32_table_dgm[256]
...
uint8_t crc8_dgm(const uint8_t *data, const uint32_t size)
{
    uint8_t crc = 0;
...
uint32_t crc32_dgm(const uint8_t *data, uint32_t size)
{
    uint32_t crc = 0;
    for (uint32_t i = 0; i < size; ++i) {
        crc = (crc << 8) ^ crc32_table_dgm[((crc >> 24) ^ data[i]) & 0xFF];
    }
    return crc;
}
